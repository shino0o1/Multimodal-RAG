...
PROMPTS_ZH["IMAGE_ANALYSIS_SYSTEM"] = (
    "你是一位农业病虫害知识抽取专家。请只提取对病虫害知识图谱有价值的信息。"
    "必须只输出一个合法JSON对象，不要Markdown代码块，不要解释文字，不要在JSON前后添加任何字符。"
)
...
2. 只输出一个合法JSON对象；不要Markdown代码块；不要解释文字；不要在JSON前后添加任何字符。
...
{{
    ...
    - detailed_description 按四段输出：[实体候选]、[属性句]、[关系线索]、[证据短句]
    - 可用于关系抽取的语义线索（如：致病、危害、发生于、使用药剂、防治、地理位置、属类隶属、生命周期）
...
{{
    ...
    - detailed_description 按四段输出：[实体候选]、[属性句]、[关系线索]、[证据短句]
    - 结合上下文补全实体指代与语义关系
    - 可用于关系抽取的语义线索（如：致病、危害、发生于、使用药剂、防治、地理位置、属类隶属）
    - 关键数值、剂量、时间范围、频次、条件等结构化信息
    - 忽略无关列、装饰性说明和与病虫害无关信息
    始终使用具体实体名、字段名和数值。",
    "entity_info": {{
